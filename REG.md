...
data* by Guillaume Coqueret) for comments on the results below, **as
well as for the presentation of the database**.
...
**THE TASK** is to predict the feeling towards unions based on the
following variables: Age, Gender, Education, Income, Church\_attendance,
Home\_ownership, Abortion, Feeling\_unions, Foreign\_policy, and
Government\_economy. **TRAINING SET**: 1986-2008, **TESTING SET**:
2012-2016.
...
Performance metric: the Mean Absolute Error, which we compute below.
    ...
    outlier.rm <- function(x){ # Removes outliers
      x[x>100] <- 100
      x[x<0] <- 0
      return(x)
    }
    fit <- lm(Feeling_unions ~ ., data = train)
    pred <- outlier.rm(predict(fit,test))
...
![](REG_files/figure-markdown_strict/regression_trees-1.png)
...
![](REG_files/figure-markdown_strict/sensi_trees-1.png)
...
![](REG_files/figure-markdown_strict/boosted_trees-1.png)
...
![](REG_files/figure-markdown_strict/svm_reg-1.png)

Neural networks
---------------

We stick to the simplest case: multilayer perceptron. We plot the
improvement of the MAE based on the number of rounds of
back-propagation.
...
![](REG_files/figure-markdown_strict/ann_reg-1.png)

**CONCLUSION**: using the training sample average to predict all
Feeling\_unions gives an MAE of 22.2. The best we could do here is 19.9,
which is only a marginal improvement. Adding more variables (e.g., party
affiliation or feelings) can help further reduces the MAE.
